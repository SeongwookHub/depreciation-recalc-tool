...
# ---------------------------------------------------------------------------
# N. 기본자산(이벤트 없음) - 이중체감법: 유형 3 + 무형 2 = 5
# ---------------------------------------------------------------------------
for i, (acq, life) in enumerate(zip(acq_variety[:3], [5, 8, 10]), start=1):
    add_normal(f"이중체감법기본자산_유형{i:02d}", "유형자산", 11_000_000 + i * 400_000, 500_000,
               life, "이중체감법", acq)
for i, (acq, life) in enumerate(zip(acq_variety[:2], [6, 12]), start=1):
    add_normal(f"이중체감법기본자산_무형{i:02d}", "무형자산", 8_500_000 + i * 300_000, 0,
               life, "이중체감법", acq)

# ---------------------------------------------------------------------------
# O. 기본자산(이벤트 없음) - 연수합계법: 유형 3 + 무형 2 = 5
# ---------------------------------------------------------------------------
for i, (acq, life) in enumerate(zip(acq_variety[:3], [5, 7, 10]), start=1):
    add_normal(f"연수합계법기본자산_유형{i:02d}", "유형자산", 10_500_000 + i * 400_000, 500_000,
               life, "연수합계법", acq)
for i, (acq, life) in enumerate(zip(acq_variety[:2], [4, 9]), start=1):
    add_normal(f"연수합계법기본자산_무형{i:02d}", "무형자산", 8_200_000 + i * 300_000, 0,
               life, "연수합계법", acq)

# ---------------------------------------------------------------------------
# P. 처분: 이중체감법 1 + 연수합계법 1 = 2
# ---------------------------------------------------------------------------
add_normal("처분_이중체감_당기중_유형01", "유형자산", 12_000_000, 0, 6, "이중체감법",
           dt.date(2021, 1, 1), disposal=dt.date(2025, 7, 31))
add_normal("처분_연수합계_당기중_유형01", "유형자산", 11_000_000, 0, 6, "연수합계법",
           dt.date(2021, 1, 1), disposal=dt.date(2025, 4, 30), mismatch_offset=180_000)

# ---------------------------------------------------------------------------
# Q. 재추정(방법 유지): 이중체감법 1 + 연수합계법 1 = 2
# ---------------------------------------------------------------------------
add_normal("재추정_이중체감_유형01", "유형자산", 13_000_000, 0, 6, "이중체감법",
           dt.date(2021, 1, 1), reest_date=dt.date(2024, 1, 1), reest_life=8)
add_normal("재추정_연수합계_유형01", "유형자산", 12_500_000, 0, 6, "연수합계법",
           dt.date(2021, 1, 1), reest_date=dt.date(2024, 1, 1), reest_life=8,
           accum_mismatch_offset=400_000)

# ---------------------------------------------------------------------------
# R. 재추정(방법변경, 기존 방법과 신규 방법 혼합) = 4
# ---------------------------------------------------------------------------
add_normal("방법변경_정액to이중체감_01", "유형자산", 12_000_000, 0, 8, "정액법",
           dt.date(2021, 1, 1), reest_date=dt.date(2024, 1, 1), reest_life=6, reest_method="이중체감법")
add_normal("방법변경_정률to연수합계_01", "유형자산", 13_500_000, 0, 8, "정률법",
           dt.date(2020, 1, 1), reest_date=dt.date(2024, 1, 1), reest_life=6, reest_method="연수합계법")
add_normal("방법변경_이중체감to정액_01", "유형자산", 11_500_000, 0, 8, "이중체감법",
           dt.date(2020, 1, 1), reest_date=dt.date(2024, 1, 1), reest_life=6, reest_method="정액법")
add_normal("방법변경_연수합계to정률_01", "유형자산", 12_800_000, 0, 8, "연수합계법",
           dt.date(2021, 1, 1), reest_date=dt.date(2024, 7, 1), reest_life=5, reest_method="정률법",
           mismatch_offset=-150_000)

# ---------------------------------------------------------------------------
# S. 자본적지출: 이중체감법 1 + 연수합계법 1 = 2
# ---------------------------------------------------------------------------
add_normal("자본적지출_이중체감_전기_01", "유형자산", 12_000_000, 0, 10, "이중체감법",
           dt.date(2020, 1, 1), capex_date=dt.date(2023, 1, 1), capex_amount=2_000_000)
add_normal("자본적지출_연수합계_전기_01", "유형자산", 11_500_000, 0, 10, "연수합계법",
           dt.date(2020, 1, 1), capex_date=dt.date(2023, 1, 1), capex_amount=1_800_000)

# ---------------------------------------------------------------------------
# T. 상각중단: 이중체감법 1 + 연수합계법 1 = 2
# ---------------------------------------------------------------------------
add_normal("상각중단_이중체감_당기일부_01", "유형자산", 9_500_000, 0, 5, "이중체감법",
           dt.date(2021, 1, 1), susp_start=dt.date(2025, 3, 1), susp_end=dt.date(2025, 9, 30))
add_normal("상각중단_연수합계_당기일부_01", "유형자산", 9_800_000, 0, 5, "연수합계법",
           dt.date(2021, 1, 1), susp_start=dt.date(2025, 2, 1), susp_end=dt.date(2025, 8, 31))
...
add_error("오류_이중체감_내용연수0", "유형자산", 10_000_000, 0, 0, "이중체감법", dt.date(2022, 1, 1), 1_000_000)
add_error("오류_연수합계_잔존가치초과", "유형자산", 10_000_000, 15_000_000, 5, "연수합계법",
          dt.date(2022, 1, 1), 500_000)
